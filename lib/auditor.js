...
export function auditMacroSeries(list, now) {
  ...
  (list || []).forEach(entry => {
    ...
    if (isFinite(lastAt)) {
      ...
      if (days > entry.maxAgeDays || days > 45) {
        out.push(finding(days > 120 ? SEVERITY.critical : SEVERITY.warning, 'macro-stale',
          ...
          '). It is being read as the current state of a market that has moved.'));
      }
    }
  });
  return out;
