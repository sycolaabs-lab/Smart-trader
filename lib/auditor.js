...
// or ends on.
// Gold's week, in UTC: it opens Sunday around 22:00 and closes Friday around
// 21:00. Excluding only Saturday and Sunday by whole days therefore counts
// Friday evening and Sunday evening as tradeable when they are not — three
// hours of phantom Friday was enough to make a normal Friday close read as a
// stalled feed all weekend.
export const MARKET_WEEK = { closesFridayUtcHour: 21, opensSundayUtcHour: 22 };

// Is this instant inside the trading week?
export function isMarketOpen(ts) {
  const d = new Date(ts);
  const dow = d.getUTCDay();       // 0 Sun ... 6 Sat
  const hour = d.getUTCHours();
  if (dow === 6) return false;                                   // Saturday
  if (dow === 0) return hour >= MARKET_WEEK.opensSundayUtcHour;   // Sunday evening open
  if (dow === 5) return hour < MARKET_WEEK.closesFridayUtcHour;   // Friday close
  return true;
}

// When does the market next open, from an instant when it is shut? Returns the
// instant itself if it is already open, so callers can use it unconditionally.
export function nextMarketOpen(ts) {
  const HOUR = 3600000;
  if (isMarketOpen(ts)) return ts;
  const d = new Date(ts);
  // Walk forward in hour steps to the Sunday open. At most ~48 steps.
  let cursor = Math.floor(ts / HOUR) * HOUR + HOUR;
  let guard = 0;
  while (!isMarketOpen(cursor) && guard++ < 200) cursor += HOUR;
  return cursor;
}

// When did the market last close, from an instant when it is shut?
export function lastMarketClose(ts) {
  const HOUR = 3600000;
  if (isMarketOpen(ts)) return ts;
  let cursor = Math.floor(ts / HOUR) * HOUR;
  let guard = 0;
  while (!isMarketOpen(cursor - HOUR) && guard++ < 200) cursor -= HOUR;
  return cursor;
}

// When does the market next shut, from an instant when it is open? Returns the
// instant itself if it is already shut, so callers can use it unconditionally.
export function nextMarketClose(ts) {
  const HOUR = 3600000;
  if (!isMarketOpen(ts)) return ts;
  let cursor = Math.floor(ts / HOUR) * HOUR + HOUR;
  let guard = 0;
...
// The most recent weekly close at or before `ts`, whether or not the market is
// currently open. lastMarketClose answers a narrower question — it only works
// while the market is shut — and "did this order live through a close?" has to
// be answerable on a Monday afternoon too.
export function previousMarketClose(ts) {
  const HOUR = 3600000;
  let cursor = Math.floor(ts / HOUR) * HOUR;
  let guard = 0;
  // Walk back to the first shut hour, then to the moment it shut.
  while (isMarketOpen(cursor) && guard++ < 400) cursor -= HOUR;
  if (guard >= 400) return null;          // no close within the window we scan
  while (!isMarketOpen(cursor - HOUR) && guard++ < 400) cursor -= HOUR;
  return cursor;
}

// Did something alive at `since` live through a weekly close by `now`?
export function spannedMarketClose(since, now) {
  // Number.isFinite throughout this file's time arithmetic: the global one says
  // null is finite, which turns a missing timestamp into 1 January 1970 — before
  // every weekly close there has ever been.
  if (!Number.isFinite(since) || !Number.isFinite(now) || !(now > since)) return false;
  const close = previousMarketClose(now);
  return close != null && close > since;
}

// Tradeable hours between two instants — weekend closure excluded.
//
// This is the honest clock for anything that ages an order. Wall-clock hours
// count the 49-hour weekend against a resting limit that had no opportunity to
// fill in any of them: a Friday-evening order is four times past a 12h fill
// limit by Sunday lunchtime without price ever having moved.
export const TRADEABLE_HOURS_PER_WEEK = 119;   // Sun 22:00 -> Fri 21:00 UTC

export function marketHoursBetween(aMs, bMs) {
  if (!Number.isFinite(aMs) || !Number.isFinite(bMs) || !(bMs > aMs)) return 0;
  const HOUR = 3600000;
  const WEEK = 7 * 24 * HOUR;
  let total = 0, cursor = aMs, guard = 0;
  // Whole weeks are exact and identical, so skip them rather than walking
  // 168 hours at a time. A months-old signal in the log would otherwise cost
  // thousands of iterations on every render.
  const wholeWeeks = Math.floor((bMs - cursor) / WEEK);
  if (wholeWeeks > 0) { total += wholeWeeks * TRADEABLE_HOURS_PER_WEEK; cursor += wholeWeeks * WEEK; }
  while (cursor < bMs && guard++ < 200000) {
    const hourStart = Math.floor(cursor / HOUR) * HOUR;
    const hourEnd = Math.min(hourStart + HOUR, bMs);
    if (isMarketOpen(hourStart)) total += (hourEnd - cursor) / HOUR;
    cursor = hourEnd;
  }
  return total;
}

// A single description of where the trading week stands right now, so the UI,
// the engine and the worker all answer the question the same way.
export function marketClock(ts) {
  const now = isFinite(ts) ? ts : Date.now();
  const open = isMarketOpen(now);
  if (open) {
    const closesAt = nextMarketClose(now);
    return { open: true, now, closedSince: null, opensAt: null, hoursUntilOpen: 0,
      closesAt, minutesUntilClose: (closesAt - now) / 60000 };
  }
  const opensAt = nextMarketOpen(now);
  return {
    open: false,
    now,
    closedSince: lastMarketClose(now),
    opensAt,
    hoursUntilOpen: (opensAt - now) / 3600000,
    closesAt: null,
    minutesUntilClose: 0
  };
}
...
export function expectedBarsExcludingWeekend(startMs, endMs, intervalMs) {
  ...
  const DAY = 86400000;
  // Walk in hour slices rather than whole days, so the Friday close and the
  // Sunday open land where they actually are instead of at midnight.
  const HOUR = 3600000;
  let total = 0;
  let cursor = startMs;
  let guard = 0;
  while (cursor < endMs && guard++ < 200000) {
    const hourStart = Math.floor(cursor / HOUR) * HOUR;
    const hourEnd = Math.min(hourStart + HOUR, endMs);
    if (isMarketOpen(hourStart)) total += (hourEnd - cursor) / intervalMs;
    cursor = hourEnd;
  }
...
// Each timeframe has to be current by ITS OWN clock.
//
// auditFreshness above watches the 15-minute feed. Nothing watched the higher
// ones — and they carry the largest weights in the composite, roughly 45 of the
// ~109 total across weekly, daily, 4H and 1H. A 4H series frozen nine days ago
// still holds a perfectly plausible gold price, so the cross-series level check
// passes, the analysis runs at full confidence, and the audit reports nothing.
// The only symptom is that higher-timeframe structure quietly stops changing.
//
// Staleness is measured in BARS MISSED rather than elapsed time, because a
// series' own cadence is what makes a gap abnormal: three hours is nothing for a
// weekly series and a long outage for a 15-minute one. Weekends are excluded, so
// a Friday close does not read as stale all Sunday.
export const SERIES_FRESHNESS = { warnBars: 6, criticalBars: 24 };

function medianGapOf(candles) {
  const gaps = [];
  for (let i = 1; i < candles.length; i++) {
...
export function auditSeriesFreshness(series, now, cfg) {
  cfg = Object.assign({}, SERIES_FRESHNESS, cfg || {});
  ...
  Object.keys(series || {}).forEach(name => {
    ...
    if (!isFinite(last.time)) return;
    const gap = medianGapOf(s);
    if (!gap) return;
    const missed = expectedBarsExcludingWeekend(last.time, t, gap) - 1;
    if (missed < cfg.warnBars) return;
    const age = t - last.time;
    out.push(finding(missed >= cfg.criticalBars ? SEVERITY.critical : SEVERITY.warning,
      'series-stale', 'The ' + name + ' series has stopped updating',
      'Its newest bar is ' + stamp(last.time) + ' — about ' + Math.round(missed) +
      ' bars behind at its own ' + Math.round(gap / 60000) + '-minute cadence (' +
      (age / 3600000).toFixed(1) + 'h, weekends excluded). Structure on this timeframe is frozen, ' +
      'and it still carries its full weight in the score.'));
  });
  return out;
}
...
export function auditFreshness(candles, now, maxAgeMs) {
  ...
  // Over the weekend the newest candle is *supposed* to be Friday's close, so
  // elapsed age says nothing. Only tradeable time counts as staleness — the
  // alternative is a critical stale-feed alarm every Saturday.
  const age = marketHoursBetween(last.time, at) * 3600000;
  if (maxAgeMs && age > maxAgeMs) {
    out.push(finding(age > maxAgeMs * 4 ? 'critical' : 'warning', 'stale-feed', 'Price data is stale',
      'Newest candle is ' + Math.round(age / 60000) + ' min old in trading time. Levels are being computed against a market that has moved on.'));
  }
  return out;
}

// ---- is the system reasoning about a market that is trading? ---------------
// The backstop for the whole weekend problem. Everything upstream is supposed
// to stand down when gold is shut; this checks, from outside, that it did.
export function auditMarketHours(result, now) {
  const out = [];
  const at = isFinite(now) ? now : Date.now();
  if (isMarketOpen(at)) return out;
  const clock = marketClock(at);
  const hrs = (clock.hoursUntilOpen).toFixed(1);
  if (result && result.direction && result.direction !== 'HOLD') {
    out.push(finding('critical', 'signal-while-closed', 'A directional signal was produced while gold is closed',
      'The market shut at ' + new Date(clock.closedSince).toISOString().slice(0, 16).replace('T', ' ') +
      ' UTC and reopens in ' + hrs + 'h. Nothing can fill, and the outcome would be graded against a market that never traded.'));
  } else {
    out.push(finding('note', 'market-closed', 'Gold is closed',
      'Reopens in ' + hrs + 'h. The feed is frozen at Friday\u2019s close, so this read is Friday\u2019s, not today\u2019s.'));
  }
  return out;
}
...
export function auditAnalysis(input) {
  ...
  const findings = []
    ...
    // Each timeframe against its own cadence, not just the 15-minute feed.
    .concat(auditSeriesFreshness(series, now))
    .concat(auditMacroSeries(macroSeries, now))
    .concat(auditFreshness(candles, now, maxAgeMs))
    .concat(auditMarketHours(result, now))
  ...
  const dataCodes = ['no-data', 'unordered', 'malformed-candles', 'impossible-bars', 'price-out-of-band',
    ...
    'synthetic-feed', 'duplicate-candles', 'gappy-feed', 'macro-stale', 'macro-frozen', 'series-stale'];
