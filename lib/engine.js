// The trading week is defined once, in the auditor. The dependency points that
// way on purpose: the auditor must never take the engine's word for anything,
// so the engine is the side that defers.
import { isMarketOpen, marketClock, marketHoursBetween, nextMarketClose,
  spannedMarketClose } from './auditor.js';
...
// CLASSIC INDICATORS
// ============================================================
// The demo feed. It has to produce candles that could actually have happened:
// the independent auditor checks every bar against its own high-low range, and
// a generator that emits impossible bars would have the demo permanently
// reporting a broken feed — and, worse, backtests running on them.
//
// The close used to be an average of the other four prices plus noise, with
// nothing keeping it inside the bar. On a narrow range the noise pushed it past
// the high or low often enough that a few percent of every series was
// impossible. So the close is drawn first and the extremes are taken around it.
export function genData(n, startPrice, intervalMs) {
  intervalMs = intervalMs || 900000; // 15min
  // A missing start price used to make p undefined, and every bar NaN, silently.
  let p = isFinite(startPrice) ? startPrice : 2000;
  const data = [], now = Date.now();
  for (let i = n; i >= 0; i--) {
    const t = now - i * intervalMs;
    const ch = (Math.random() - 0.495) * 2.2;
    p = Math.max(1500, p + ch);
    const o = p - (Math.random() - 0.5) * 1.2;
    const c = (o + p) / 2 + (Math.random() - 0.5) * 0.9;
    const h = Math.max(o, c, p) + Math.random() * 1.8;
    const l = Math.min(o, c, p) - Math.random() * 1.8;
...
export function getSessionInfo(ms) {
  // The market being shut is a session state, not a missing one. Reporting
  // "London-NY Overlap" at Sunday lunchtime — which is what an hour-of-day
  // lookup does — presents a frozen Friday-close feed as a live one.
  if (!isMarketOpen(ms)) {
    const clock = marketClock(ms);
    return { session: 'Closed', killZone: null, marketOpen: false,
      opensAt: clock.opensAt, hoursUntilOpen: clock.hoursUntilOpen };
  }
  ...
  return { session, killZone, marketOpen: true, opensAt: null, hoursUntilOpen: 0 };
}
export function computeSessionStats(ltf) {
  const buckets = { Asian: [], London: [], 'London-NY Overlap': [], 'New York': [] };
  // Closed-market bars have no session bucket; they are excluded rather than
  // silently widening one of the four real sessions.
  ltf.forEach(c => { const { session } = getSessionInfo(c.time); if (buckets[session]) buckets[session].push(c.high - c.low); });
  const avgRange = {};
...
// Correlation between gold and one driver, aligned on the CALENDAR rather than
// by array position.
//
// pearsonCorrelation zips two arrays by index. Gold trades Sunday evening to
// Friday evening; FRED publishes on US business days and skips federal
// holidays. So the two series have different calendars, and lining them up from
// the right means every holiday in the window shifts the pairing by a day for
// everything before it. The number that comes out is not the correlation
// between gold and the driver; it is the correlation between gold and the
// driver offset by an amount that varies through the window. It always looks
// like a plausible correlation, which is what makes it dangerous.
//
// Returns null rather than a bad number when there is too little overlap.
export function correlateByDay(goldSeries, driverSeries, kind) {
  const aligned = alignByDay([goldSeries || [], driverSeries || []]);
  if (aligned.days.length < 6) return null;
  const goldChanges = toChanges(aligned.columns[0], 'price');
  const driverChanges = toChanges(aligned.columns[1], kind || 'price');
  // A series that only moves at the far end of floating point is not moving.
  // Pearson's denominator is not exactly zero there, so it happily returns a
  // correlation computed entirely from accumulated rounding error — a number
  // with no meaning that reads exactly like a weak relationship.
  if (isEffectivelyConstant(driverChanges) || isEffectivelyConstant(goldChanges)) return null;
  return pearsonCorrelation(goldChanges, driverChanges);
}

function isEffectivelyConstant(values) {
  if (!values || values.length < 2) return true;
  let lo = Infinity, hi = -Infinity, scale = 0;
  ...
  // Relative to the size of the values themselves, so this scales with the
  // instrument rather than assuming a magnitude.
  return scale > 0 && spread / scale < 1e-6;
}

// Latest period-over-period move of an aligned pair's driver, so the score and
// the correlation are computed from the same days.
export function alignedLatestChange(goldSeries, driverSeries, kind) {
  const aligned = alignByDay([goldSeries || [], driverSeries || []]);
  if (aligned.days.length < 2) return null;
  const changes = toChanges(aligned.columns[1], kind || 'price');
...
export function sessionRegimeQuality(sessionInfo, regimeInfo) {
  const sessionQuality = sessionInfo.session === 'Closed' ? 0
    : (sessionInfo.session === 'London' || sessionInfo.session === 'London-NY Overlap' || sessionInfo.session === 'New York') ? 1 : 0.3;
...
// ------------------------------------------------------------
// One cap of 500, shared between two completely different kinds of example,
// was quietly destroying the valuable half.
//
// A backtest cycle contributes about 22 out-of-sample trades and runs every
// four hours — roughly 132 a day. Live resolved trades arrive at maybe a dozen.
// Sharing one 500-slot window, the arithmetic is brutal: after two weeks of
// running, 168 real outcomes had been recorded and 36 survived. Ninety-three
// per cent of what the model trained on was synthetic, and every hard-won live
// result was evicted within days by trades that can be regenerated on demand.
//
// So they get separate budgets. Live outcomes are scarce and irreplaceable and
// are kept in bulk; backtest examples are regenerable, so a rolling window is
// all they need and they can never displace a real one.
//
// The remaining limit is on TRAINING, not memory. Boosting here costs roughly
// quadratic time — measured at 1.5s for 2,000 examples, 5.6s for 5,000, 50s for
// 20,000 — so a training pass takes a bounded sample while the store keeps
// everything. Live examples fill that sample first; backtest examples only
// occupy slots live data has not claimed.
// There is no point at which learning stops. The only hard limit in reach is
// physical — browser storage is about 5MB, shared with the signal log and the
// knowledge base — and at ~12 live outcomes a day a verbose store crosses 1MB
// somewhere around year three.
//
// So when the store gets large it is THINNED, never truncated. Truncating drops
// the oldest history entirely and the system forgets its own beginning. Thinning
// halves the resolution of the oldest half and keeps the newest intact, so the
// record still spans every day it has ever run — just coarser the further back
// you look, the way memory actually works. Applied repeatedly it settles near
// the soft limit while covering unbounded time.
export const META_LIMITS = {
  liveSoftLimit: 12000,   // ~1MB verbose, ~3 years at full resolution before any thinning
  backtestStore: 1500,    // regenerable, so a rolling window suffices
  trainingSet: 2000,      // measured ~1.5s to train; a cost ceiling, not a memory one
  recentShare: 0.75       // of the training slots, reserved for the most recent examples
};

// Halve the resolution of the oldest half, leave the newest half untouched.
// Nothing is dropped from the end of history — the span is preserved, the
// detail within the old part is not.
export function thinExamples(list, softLimit) {
  const arr = Array.isArray(list) ? list : [];
  const limit = softLimit > 0 ? softLimit : META_LIMITS.liveSoftLimit;
  if (arr.length <= limit) return arr;
  const keepFrom = Math.floor(arr.length / 2);
  const recent = arr.slice(keepFrom);
  const older = arr.slice(0, keepFrom).filter((_, i) => i % 2 === 0);
  return older.concat(recent);
}

// Choose what one training pass sees.
//
// Live first, because a real outcome strictly dominates a simulated one. Within
// the live examples, most slots go to recent history — the market this model
// has to work in now — but a share is spread evenly across everything older, so
// the accumulated record still informs the model instead of being discarded the
// moment it scrolls out of the recent window.
export function buildMetaTrainingSet(live, backtest, cfg) {
  cfg = Object.assign({}, META_LIMITS, cfg || {});
  const liveArr = Array.isArray(live) ? live : [];
  const btArr = Array.isArray(backtest) ? backtest : [];
  const budget = Math.max(1, cfg.trainingSet);

  let chosenLive;
  if (liveArr.length <= budget) {
    chosenLive = liveArr.slice();
  } else {
    const recentCount = Math.floor(budget * cfg.recentShare);
    const olderCount = budget - recentCount;
    const recent = liveArr.slice(-recentCount);
    const older = liveArr.slice(0, liveArr.length - recentCount);
    // Evenly spaced across the older history rather than a contiguous block, so
    // the sample spans every regime the record has seen.
    const stride = older.length / olderCount;
    const sampled = [];
    for (let i = 0; i < olderCount; i++) {
      const idx = Math.min(older.length - 1, Math.floor(i * stride));
      sampled.push(older[idx]);
    }
    chosenLive = sampled.concat(recent);
  }

  const room = budget - chosenLive.length;
  const chosenBacktest = room > 0 ? btArr.slice(-room) : [];
  return {
    examples: chosenBacktest.concat(chosenLive),
    liveUsed: chosenLive.length,
    backtestUsed: chosenBacktest.length,
    liveStored: liveArr.length,
    backtestStored: btArr.length
  };
}
...
export function resolveSignal(signal, candles, cfg) {
  ...
  const nowMs = Number.isFinite(cfg.now) ? cfg.now : (replaying ? latestCandle : wallNow);
  ...
  //
  // The age that matters is tradeable hours, not elapsed hours. Gold is shut
  // for 49 hours every weekend; counting those against a 12h fill limit kills
  // every order alive at Friday's close before Monday's first tick, without
  // price ever having had the chance to reach it — and records the cancellation
  // as evidence the level was wrong. Replays use elapsed time as before, since
  // the archive's own timestamps already carry the weekend gap.
  const timeIsUsable = isFinite(signalTime) && signalTime > 0;
  const ageHours = timeIsUsable
    ? (replaying ? (nowMs - signalTime) / HOUR : marketHoursBetween(signalTime, nowMs))
    : 0;
  ...
  const openHours = filledAtMs > 0
    ? (replaying ? (nowMs - filledAtMs) / HOUR : marketHoursBetween(filledAtMs, nowMs))
    : 0;
  if (filledAtMs > 0 && cfg.maxHoursOpen > 0 && openHours >= cfg.maxHoursOpen) {
    return { status: 'expired', reason: 'position scratched after ' + Math.round(openHours) +
...
export function autonomyGate(result, plan, openSignals, lastSignalTime, cfg) {
  ...
  if (lastSignalTime) {
    // cfg.now so the cooldown can be tested without the machine's clock; every
    // other time-aware function in here already takes one.
    const mins = ((isFinite(cfg.now) ? cfg.now : Date.now()) - lastSignalTime) / 60000;
...
export function signalLiveness(sig, cfg, opts) {
  ...
  // Tradeable hours, matching the clock the kill switch actually ages against —
  // otherwise the badge counts down through a weekend the kill switch pauses for.
  const ageHours = isFinite(start) && start > 0 ? marketHoursBetween(start, now) : null;
  ...
  // While the market is shut the clock is genuinely paused, and saying so is the
  // difference between "your order is nearly dead" and "nothing can happen yet".
  if (!isMarketOpen(now)) {
    const clock = marketClock(now);
    label += ' · market shut';
    detail += ' Market is closed — the stale-order clock is paused and resumes in ' + hrs(clock.hoursUntilOpen) + '.';
  }
...
export const PAPER_DEFAULTS = {
  ...
  maxConcurrent: 3,
  // ---- lots ---------------------------------------------------------------
  // Sizing was in troy ounces, which is the correct internal unit but not the
  // one anyone trades in. A broker quotes XAUUSD in lots: one standard lot is
  // 100 oz, and orders move in 0.01-lot steps. "5.26 units" is not a number you
  // can place anywhere.
  //
  // Rounding to a real lot step also makes the simulation honest. A position
  // cannot be sized to arbitrary precision, so the risk actually taken differs
  // slightly from the risk requested — every real account works this way, and
  // an account that pretends otherwise reports a P&L nobody could have made.
  contractSize: 100,     // troy oz in one standard lot
  lotStep: 0.01,         // smallest increment a broker will accept
  minLots: 0.01,
  // 'risk' sizes from the stop so each trade risks the same percentage.
  // 'fixed' places the same lot size every time and lets the risk vary.
  sizingMode: 'risk',
  fixedLots: 0.10
};

// Round a lot count down to a placeable size. Down, not nearest: rounding up
// would risk more than asked for, and being slightly under the intended risk is
// the side to err on.
export function roundToLotStep(lots, step) {
  const s = step > 0 ? step : 0.01;
  const rounded = Math.floor(lots / s + 1e-9) * s;
  // Floating point leaves 0.30000000000000004 behind; lots are quoted to the
  // step, so quantise to it.
  const decimals = Math.max(0, Math.ceil(-Math.log10(s)) + 1);
...
// Size a position, in lots, and report the risk that size actually carries.
//
// The requested risk and the taken risk are different numbers once lots are
// rounded to something a broker would accept, and both are returned: the second
// is what the account is really exposed to, and it is the one the P&L is built
// from.
export function paperPositionSize(balance, riskPercent, entryFill, stopPrice, cfg) {
  ...
  const contract = cfg.contractSize > 0 ? cfg.contractSize : 100;

  let lots;
  const requestedRisk = balance * (riskPercent / 100);
  if (cfg.sizingMode === 'fixed') {
    lots = roundToLotStep(cfg.fixedLots, cfg.lotStep);
  } else {
    if (!isFinite(requestedRisk) || requestedRisk <= 0) return null;
    lots = roundToLotStep(requestedRisk / (stopDistance * contract), cfg.lotStep);
  }
  // Below the broker's minimum there is no trade to place — the honest answer
  // is that the account cannot take this setup, not a fractional position no
  // venue would fill.
  if (!isFinite(lots) || lots < (cfg.minLots || 0) || lots <= 0) return null;

  const units = lots * contract;
  // What the account is actually risking after rounding, which is what the P&L
  // and the R-multiples are computed against.
  const riskAmount = units * stopDistance;
  if (!isFinite(units) || units <= 0) return null;
  return { units, lots, riskAmount, requestedRisk, stopDistance, contractSize: contract };
}
...
// Why the paper account would refuse this signal, in words. Returns null when it
// would take it.
//
// openPaperPosition returns null for several different reasons and the caller
// could not tell them apart, so a trade that never opened looked identical to
// one that was never attempted — you press Generate, nothing happens, and
// nothing says why. Every refusal now has a sentence.
export function paperRejectReason(signal, account, cfg) {
  cfg = Object.assign({}, PAPER_DEFAULTS, cfg || {});
  if (!signal) return 'no signal to trade';
  if (signal.dir !== 'BUY' && signal.dir !== 'SELL') return 'the engine has no direction — nothing to trade';
  const committed = (account && account.positions || []).filter(p => p.status === 'open' || p.status === 'pending');
  // One live position per signal. Without this the same signal arriving twice —
  // a worker snapshot re-delivering an id that fell off the end of a trimmed
  // log, say — silently doubles the exposure, and both copies then accrue
  // floating P&L against the same trade.
  if (signal.id != null && committed.some(p => p.signalId === signal.id)) {
    return 'this signal already has a live paper position';
  }
  if (committed.length >= cfg.maxConcurrent) {
    return 'the book is full — ' + committed.length + ' position(s) already open or resting, cap is ' +
      cfg.maxConcurrent + ' (raise "Max concurrent" in paper settings)';
  }
  if (!(account && account.balance > 0)) return 'the paper account has no balance left';
  // The stop has to be a real stop BEFORE the spread is applied. With entry and
  // stop at the same price the spread alone leaves a sliver of a gap, and since
  // size is risk divided by stop distance, that sliver sizes an enormous
  // position off a plan that is plainly malformed.
  if (!(Math.abs(signal.entry - signal.sl) > 0)) {
    return 'the plan has no stop — entry and stop are the same price';
  }
  const entryFill = worsePrice(signal.entry, signal.dir, 'entry', cfg.spreadPips, cfg.pipSize);
  const sized = paperPositionSize(account.balance, cfg.riskPercent, entryFill, signal.sl, cfg);
  if (!sized) {
    // Almost always this one: the stop is wide enough, or the account small
    // enough, that the risk budget buys less than the broker's minimum lot.
    const stopDistance = Math.abs(entryFill - signal.sl);
    const contract = cfg.contractSize > 0 ? cfg.contractSize : 100;
    const wanted = (account.balance * (cfg.riskPercent / 100)) / (stopDistance * contract);
    if (isFinite(wanted) && wanted > 0 && wanted < (cfg.minLots || 0.01)) {
      return 'the trade would need ' + wanted.toFixed(4) + ' lots, below the ' + (cfg.minLots || 0.01) +
        ' minimum — the stop is too wide for this balance at ' + cfg.riskPercent +
        '% risk (raise the risk percentage or the starting balance)';
    }
    return 'the trade cannot be sized — the risk percentage leaves nothing to stake';
  }
  return null;
}
...
export function openPaperPosition(signal, account, cfg) {
  ...
  // A plan whose stop sits on the entry is malformed; the spread would otherwise
  // supply a fake stop distance and size an enormous position off it.
  if (!(Math.abs(signal.entry - signal.sl) > 0)) return null;
  const entryFill = worsePrice(signal.entry, signal.dir, 'entry', cfg.spreadPips, cfg.pipSize);
  const sized = paperPositionSize(account.balance, cfg.riskPercent, entryFill, signal.sl, cfg);
  ...
  return {
    ...
    units: sized.units,
    lots: sized.lots,
    contractSize: sized.contractSize,
    riskAmount: sized.riskAmount,
    // What was asked for, before the lot step rounded it. Kept so the account
    // can show the gap rather than quietly reporting the rounded figure as the
    // intended one.
    requestedRisk: sized.requestedRisk,
...
export function unrealisedPnl(position, price) {
  // Number.isFinite: the global one accepts null, and a null mark then books
  // the whole position value as a loss — a $10,000 account showed -$20,000 of
  // floating P&L from one missing price.
  if (!position || position.status !== 'open' || !Number.isFinite(price)) return 0;
  const direction = position.dir === 'BUY' ? 1 : -1;
...
// An epoch that is not a usable timestamp means "no reset has happened". The
// UI used to re-derive this with `!paper.epoch` and a bare `>=`, which agrees
// with this for null and disagrees for everything else: a stored ISO string
// made the summary count three closed trades while the list below it showed
// none. One definition, exported, used by both.
export function normaliseEpoch(epoch) {
  const n = typeof epoch === 'string' ? Date.parse(epoch) : epoch;
  return isFinite(n) && n > 0 ? n : null;
}

export function isInEra(position, epoch) {
  const since = normaliseEpoch(epoch);
  if (since == null) return true;
  if (!position) return false;
  const t = Date.parse(position.closedAt || position.openedAt || '');
  return (isFinite(t) ? t : 0) >= since;
}

// Did the market answer the question this trade asked?
//
// Only then is it evidence. A position closed by a rule — flattened for news or
// the weekend, scratched by the kill switch, settled by an account reset — moved
// money and belongs in the balance, but counting it as a win or a loss would
// report a verdict nobody reached. `closeReason` is what marks those, and it has
// to be checked before the legacy fallback below: a reset closes at the mark, so
// its P&L is usually non-zero and the fallback would grade it.
export function isGradedPosition(p) {
  if (!p) return false;
  if (p.outcome === 'won' || p.outcome === 'lost') return true;
  if (p.closeReason) return false;
  // Records written before outcomes were tagged: a non-zero P&L means it ran to
  // a stop or a target, because nothing else closed a position back then.
  return p.outcome == null && (p.pnl || 0) !== 0;
}

// `epoch` is when the account was last reset (ms, or null for never).
//
// Resetting used to delete every position, which threw away the one thing the
// paper trader exists to produce: a record of how the system's own signals
// actually turned out. That record is training data — the meta-labeler, the
// journal, the condition breakdown and the win rate all read it — so wiping it
// to zero the balance destroys evidence to fix a display.
//
// So a reset now moves a line rather than deleting anything. Positions closed
// before the line stay in the array, keep feeding everything that learns from
// them, and are simply not counted in the running balance, equity curve or
// drawdown. `lifetime` reports the full record alongside, so the distinction is
// visible instead of implied.
export function paperAccountSummary(positions, startingBalance, price, epoch) {
  const all = positions || [];
  const since = normaliseEpoch(epoch);
  const era = all.filter(p => isInEra(p, since));
  const archived = all.filter(p => !isInEra(p, since));
  const closed = era.filter(p => p.status === 'closed');
  const open = era.filter(p => p.status === 'open');       // filled, carrying exposure
  const pending = era.filter(p => p.status === 'pending');  // resting orders, no exposure
  const cancelled = era.filter(p => p.status === 'cancelled');
  ...
  // Money and verdict are different questions.
  //
  // Every closed position moved real money and belongs in the balance, the
  // equity curve and the drawdown. But only a trade that reached its own stop
  // or target is EVIDENCE about the setup. A position closed early — flattened
  // ahead of NFP, scratched by the kill switch — was cut short by a rule, not
  // by the market answering the question the setup asked. Counting it in the
  // win rate would say the analysis was wrong when nothing of the sort was
  // established.
  const graded = closed.filter(isGradedPosition);
  const cutShort = closed.filter(p => graded.indexOf(p) === -1);

  const wins = graded.filter(p => (p.pnl || 0) > 0);
  const losses = graded.filter(p => (p.pnl || 0) < 0);
  ...
  return {
    ...
    // Closed, but cut short by a rule rather than resolved by the market.
    cutShortCount: cutShort.length,
    ...
    avgR: graded.length ? graded.reduce((s, p) => s + (p.rMultiple || 0), 0) / graded.length : null,
    // Everything the account has ever done, reset or not. This is the number
    // that answers "how good are the signals", and a reset must not touch it.
    epoch: since,
    archivedCount: archived.length,
    lifetime: lifetimeRecord(all)
  };
}

// The full record across every era. Deliberately reports only what survives a
// reset: outcomes the market actually delivered. Balances and drawdowns are
// era-scoped by definition, so they are not here.
export function lifetimeRecord(positions) {
  ...
  return {
    closedCount: closed.length,
    gradedCount: graded.length,
    cutShortCount: closed.length - graded.length,
    wins: wins.length,
    losses: losses.length,
    winRate: graded.length ? wins.length / graded.length : null,
    profitFactor: grossLoss > 0 ? grossWin / grossLoss : (grossWin > 0 ? Infinity : null),
    avgR: graded.length ? graded.reduce((s, p) => s + (p.rMultiple || 0), 0) / graded.length : null,
    netPnl: closed.reduce((s, p) => s + (p.pnl || 0), 0)
  };
}

// Which live paper positions no longer match the signal log?
//
// Nothing reconciled these. A position is closed by the same event that resolves
// its signal, so as long as both happen in one pass they stay in step — but they
// do not always. The signal log syncs across devices and survives a closed tab;
// the paper book is local and does not. So a trade resolved by the worker while
// this tab was shut comes back as a signal that is already `won`, and the diff
// that closes positions only looks at what changed in THIS merge, finds nothing
// new, and leaves the position open. It then accrues floating P&L forever
// against a trade the system considers finished.
//
// Measured on a seeded book: three positions whose signals had already resolved
// showed -$3,951 of floating loss and dragged a $10,000 account to $6,049 of
// equity, while realised balance sat untouched at $10,000 and "Closed trades"
// read 0.
//
// Returns what to do rather than doing it, so the browser can route each through
// its own close path and the result stays testable.
export function reconcilePaperBook(positions, signals) {
  const byId = new Map((signals || []).filter(s => s && s.id != null).map(s => [s.id, s]));
  const seenLive = new Set();
  const out = [];
  (positions || []).forEach(p => {
    if (!p || (p.status !== 'open' && p.status !== 'pending')) return;
    const sig = byId.get(p.signalId);
    if (!sig) {
      // No signal at all: nothing will ever close it.
      out.push({ id: p.id, signalId: p.signalId, action: 'cancel', outcome: 'expired',
        reason: 'no signal in the log matches this position' });
      return;
    }
    if (sig.status === 'won' || sig.status === 'lost') {
      out.push({ id: p.id, signalId: p.signalId, action: 'close', outcome: sig.status,
        exitPrice: sig.exitPrice,
        reason: 'its signal resolved as ' + sig.status + ' while this position stayed open' });
      return;
    }
    if (sig.status === 'expired') {
      out.push({ id: p.id, signalId: p.signalId, action: 'cancel', outcome: 'expired',
        reason: 'its signal was cancelled but this position stayed open' });
      return;
    }
    // Live signal, live position — but only one of them may be live at a time.
    const key = p.signalId;
    if (seenLive.has(key)) {
      out.push({ id: p.id, signalId: p.signalId, action: 'cancel', outcome: 'expired',
        reason: 'duplicate live position for one signal' });
      return;
    }
    seenLive.add(key);
  });
...
// What a reset does to the book, as a pure function so it can be tested and so
// the browser and the worker cannot drift apart on it.
//
// Anything still live is closed at the mark first — an open position cannot
// straddle a reset without its P&L landing in an era it was not opened in — and
// then the line is drawn. Nothing is deleted.
export function resetPaperAccount(positions, price, now) {
  const at = isFinite(now) ? now : Date.now();
  ...
  const out = (positions || []).map(p => {
    ...
    settled++;
    if (p.status === 'pending') {
      return Object.assign({}, p, { status: 'cancelled', closedAt: iso,
        closeReason: 'account reset', pnl: 0, outcome: null });
    }
    const pnl = isFinite(price) ? unrealisedPnl(p, price) : 0;
    return Object.assign({}, p, { status: 'closed', closedAt: iso, exitPrice: price,
      closeReason: 'account reset', pnl, outcome: null });
  });
  // The epoch is one millisecond past the settlement stamp, so the positions
  // just closed by the reset fall on the archived side of the line rather than
  // opening the new era with the old era's P&L.
  return { positions: out, epoch: at + 1, settled };
}
...
// A reset no longer prunes the book, so this is the only thing standing between
// the position history and localStorage. It thins rather than truncates, for
// the same reason the meta store does: a hard cap deletes the oldest evidence
// first, which is exactly the evidence that says whether the system has
// actually improved. Measured at 453 bytes a position, so 2000 is ~885KB —
// roughly five months at a dozen trades a day before any thinning at all, and
// the very first trade survives every pass after that.
export const PAPER_HISTORY_SOFT_LIMIT = 2000;

// Positions are stored newest-first. thinExamples works oldest-first, so this
// flips, thins and flips back rather than duplicating the halving logic.
export function thinPositions(positions, softLimit) {
  const arr = Array.isArray(positions) ? positions : [];
  const limit = softLimit > 0 ? softLimit : PAPER_HISTORY_SOFT_LIMIT;
  if (arr.length <= limit) return arr;
  return thinExamples(arr.slice().reverse(), limit).reverse();
}
...
export const STORE_CAPS = {
  ...
  shadowLog: 250,
  metaExamples: 500,
  paperPositions: PAPER_HISTORY_SOFT_LIMIT
};
...
export const NEWS_WINDOW_DEFAULTS = {
  ...
  // already open rides straight into the print, and NFP routinely moves gold
  // further in ninety seconds than a normal stop is wide — with the spread
  // widening at the same moment, so the stop fills far past its level. The
  // outcome of that trade says nothing about whether the setup was any good.
  //
  // So open positions are closed and resting orders cancelled before the
  // release. Sooner than the block window, because the last few minutes before
  // a major print are already thin.
  flattenEnabled: true,
  flattenMin: 15,             // close everything this long before a high-impact release
  flattenImpacts: ['high'],
  // How far ahead to raise a visible alert, so a person watching has warning
  // rather than discovering it when positions vanish.
  alertMin: 60
};
...
export function newsWindowState(calendar, now, cfg) {
  ...
  // The next release worth flattening for, and whether we are close enough to
  // do it. Only events AHEAD of us count: once a release has printed there is
  // nothing left to get out of the way of, and re-flattening afterwards would
  // close positions for an event that has already happened.
  const flattenImpacts = cfg.flattenImpacts || cfg.blockImpacts || ['high'];
  let flattenFor = null;
  if (cfg.enabled && cfg.flattenEnabled && cfg.flattenMin > 0) {
    for (const ev of upcoming) {
      if (flattenImpacts.indexOf(ev.impact) === -1) continue;
      const minutesUntil = (ev.at - t) / 60000;
      if (minutesUntil <= cfg.flattenMin) {
        flattenFor = Object.assign({}, ev, { minutesUntil });
        break;
      }
    }
  }

  // A visible warning further out, so positions do not simply disappear on
  // someone watching the screen.
  let alertFor = null;
  if (cfg.enabled && cfg.alertMin > 0) {
    for (const ev of upcoming) {
      if (flattenImpacts.indexOf(ev.impact) === -1) continue;
      const minutesUntil = (ev.at - t) / 60000;
      if (minutesUntil <= cfg.alertMin) {
        alertFor = Object.assign({}, ev, { minutesUntil });
        break;
      }
    }
  }
  ...
  return {
    blocked: !!(active && active.blocking),
    active, next,
    flatten: !!flattenFor,
    flattenRelease: flattenFor,
    alert: !!alertFor,
    alertRelease: alertFor
  };
}

// Which live signals must be closed out ahead of a release, and why.
//
// A separate function from the resolver because this is not a judgement about
// the trade — the setup may be perfectly good. It is a scheduled event taking
// the decision out of the market's hands, and the record should say exactly
// that rather than dressing it up as an outcome.
// ============================================================
// THE WEEKEND FLATTEN
// ------------------------------------------------------------
// Nothing is carried across the close. Two separate reasons, and both are
// about the quality of the evidence rather than about the money:
//
//   * A filled position rides the Sunday gap. Gold can reopen well past a stop,
//     and the fill comes back at whatever the gap gives — so the recorded
//     outcome measures the gap, not the setup. The paper trader exists to tell
//     the system whether its own read was right; a weekend gap answers a
//     different question and then gets filed as if it answered that one.
//
//   * A resting order is no safer. On Monday it fills at its limit price in the
//     simulation, when in reality price gapped straight through and the fill
//     would have been far worse. That is a fabricated entry, and every number
//     downstream of it — R multiple, win rate, the meta-labeler's training row —
//     inherits the fabrication.
//
// So both are cleared before the close, while there is still liquidity to clear
// them into, and both are recorded as cut short rather than graded. Nothing is
// hidden: they stay in the log, tagged weekend-flatten, excluded from the win
// rate for the same reason a news flatten is.
export const WEEKEND_FLATTEN_DEFAULTS = {
  enabled: true,
  beforeMin: 20      // clear the book this long before the Friday close
};

export function weekendFlattenState(now, cfg) {
  cfg = Object.assign({}, WEEKEND_FLATTEN_DEFAULTS, cfg || {});
  const t = isFinite(now) ? now : Date.now();
  // `enabled` travels with the state, because there are now two ways a trade can
  // qualify — being inside the window, and having lived through a close — and
  // the toggle has to switch off both, not just the one it was written for.
  if (!cfg.enabled) {
    return { enabled: false, flatten: false, closed: !isMarketOpen(t), minutesUntilClose: null, closesAt: null };
  }
  // Already shut: anything still open should never have survived, so the sweep
  // still fires. A tab that was asleep through Friday evening is exactly the
  // case this has to cover.
  if (!isMarketOpen(t)) {
    return { enabled: true, flatten: true, closed: true, minutesUntilClose: 0, closesAt: null };
  }
  const closesAt = nextMarketClose(t);
  const minutesUntilClose = (closesAt - t) / 60000;
  return {
    enabled: true,
    flatten: minutesUntilClose <= cfg.beforeMin,
    closed: false,
    minutesUntilClose,
    closesAt
  };
...
export function signalsToFlattenForWeekend(signals, state, now) {
  if (state && state.enabled === false) return [];
  const live = (signals || []).filter(s => s && (s.status === 'pending' || s.status === 'open'));
  if (!live.length) return [];
  const at = isFinite(now) ? now : (state && state.now) || Date.now();
  const inWindow = !!(state && state.flatten);
  const mins = Math.max(0, Math.round((state && state.minutesUntilClose) || 0));
  const when = (state && state.closed)
    ? 'after the weekly close'
    : mins + ' min before the weekly close';

  return live
    // Two ways a trade qualifies. Being inside the window is the ordinary one.
    // The other is having lived through a close already — which is what happens
    // when the tab was shut on Friday evening and opened again on Monday. That
    // order looked fine at every moment anyone was watching, and the first thing
    // Monday's feed does is fill it at its limit price on a gap that traded
    // straight through it. Measured on a Friday 1950 buy limit against a Sunday
    // reopen at 1930, the engine books it as a LOSS at the stop: a fabricated
    // entry, a fabricated outcome, and a training row saying the level was wrong.
    .filter(s => inWindow || spannedMarketClose(signalTimeMs(s), at))
    .map(s => ({
      id: s.id,
      status: s.status,
      reason: (s.status === 'open' ? 'closed at market ' : 'order cancelled ') +
        (inWindow ? when : 'because it was still live across the weekly close') +
        ' — nothing is carried through the weekend gap, so this is not a verdict on the setup'
    }));
}

// A signal's own timestamp in ms, whichever form it was stored in.
function signalTimeMs(s) {
  if (!s) return NaN;
  const t = typeof s.time === 'string' ? Date.parse(s.time) : s.time;
  // Number.isFinite, not the global: isFinite(null) is true, because null
  // coerces to 0. A signal with no timestamp would read as 1 January 1970,
  // which is before every weekly close there has ever been, so it would be
  // cleared as having lived through one.
  return Number.isFinite(t) ? t : NaN;
}

// One line for the banner, or null when there is nothing to say.
export function weekendFlattenText(state) {
  if (!state || !state.flatten) return null;
  if (state.closed) return 'Market closed — the book is flat until gold reopens.';
  const mins = Math.max(0, Math.round(state.minutesUntilClose || 0));
  return 'Weekly close in ' + mins + ' min — closing all trades. Nothing is carried through the weekend gap.';
}

export function signalsToFlatten(signals, state) {
  if (!state || !state.flatten || !state.flattenRelease) return [];
  const ev = state.flattenRelease;
  const mins = Math.max(0, Math.round(ev.minutesUntil));
  return (signals || [])
    .filter(s => s && (s.status === 'pending' || s.status === 'open'))
    .map(s => ({
      id: s.id,
      status: s.status,
      reason: (s.status === 'open' ? 'closed at market ' : 'order cancelled ') + mins +
        ' min before ' + ev.name + ' — a scheduled release, not a verdict on the setup',
      release: ev.name,
      releaseAt: ev.at
    }));
}

// One line saying what is about to happen and what the system will do about it.
export function newsAlertText(state, cfg) {
  cfg = Object.assign({}, NEWS_WINDOW_DEFAULTS, cfg || {});
  const ev = (state && (state.flattenRelease || state.alertRelease)) || null;
  if (!ev) return null;
  const mins = Math.max(0, Math.round(ev.minutesUntil));
  const when = mins < 1 ? 'in under a minute' : mins === 1 ? 'in 1 minute' : 'in ' + mins + ' minutes';
  if (state.flatten) {
    return ev.name + ' ' + when + ' — closing open positions and cancelling resting orders now.';
  }
  const willFlatten = cfg.flattenEnabled && cfg.flattenMin > 0
    ? ' Positions will be closed ' + cfg.flattenMin + ' minutes before it.'
    : '';
  return ev.name + ' ' + when + '.' + willFlatten;
}
