...
export function autonomyGate(result, plan, openSignals, lastSignalTime, cfg) {
  cfg = Object.assign({}, AUTONOMY_DEFAULTS, cfg || {});
  if (!result || result.direction === 'HOLD') return { take: false, reason: 'no directional edge — engine is flat' };
  if (!plan || !isFinite(plan.entry) || !isFinite(plan.sl) || !isFinite(plan.tp)) return { take: false, reason: 'no valid trade plan could be built' };
  if (plan.rr <= 0) return { take: false, reason: 'trade plan has non-positive R:R' };
  if (result.confidence < cfg.minConfidence) {
    return { take: false, reason: 'confidence ' + result.confidence + '% is below the ' + cfg.minConfidence + '% autonomous threshold' };
  }
  const grade = result.fusion ? downgradeGrade(result.fusion.grade, plan.metaScore || 0) : null;
  if (grade && cfg.allowedGrades.indexOf(grade) === -1) {
    return { take: false, reason: 'grade ' + grade + ' is outside the autonomous set (' + cfg.allowedGrades.join('/') + ')' };
  }
  if ((plan.metaScore || 0) < cfg.minMetaScore) {
    return { take: false, reason: 'meta-labeler score ' + (plan.metaScore || 0).toFixed(2) + ' vetoed this setup' };
  }
  const open = (openSignals || []).filter(s => s.status === 'pending' || s.status === 'open');
  if (open.length >= cfg.maxOpenSignals) {
    return { take: false, reason: open.length + ' signals already open (cap ' + cfg.maxOpenSignals + ')' };
  }
  if (open.some(s => s.dir === result.direction)) {
    return { take: false, reason: 'already holding an open ' + result.direction + ' signal' };
  }
  if (lastSignalTime) {
    const mins = (Date.now() - lastSignalTime) / 60000;
    if (mins < cfg.cooldownMinutes) {
      return { take: false, reason: 'cooldown — ' + Math.ceil(cfg.cooldownMinutes - mins) + ' min until the next signal is allowed' };
    }
  }
  return { take: true, reason: 'confluence, grade and meta-score all cleared the autonomous thresholds', grade };
}
...
// Fraction of the daily cap at which each priority stops being funded. Low-value
// calls yield early so the analysis path still has budget at 23:00.
const QUOTA_CEILING = { low: 0.55, normal: 0.85, critical: 1.0 };
...
export function canSpend(state, dailyCap, priority, cost) {
  cost = cost || 1;
  const { used, cap } = quotaStatus(state, dailyCap);
  const ceiling = QUOTA_CEILING[priority] != null ? QUOTA_CEILING[priority] : QUOTA_CEILING.normal;
  return (used + cost) <= cap * ceiling;
}
...
export function quotaSummary(state, dailyCap) {
  const s = quotaStatus(state, dailyCap);
  const pct = Math.round(s.fraction * 100);
  let note;
  if (s.exhausted) note = 'Daily budget spent — calls are paused until 00:00 UTC so the background worker keeps its share.';
  else if (s.fraction >= QUOTA_CEILING.normal) note = 'Budget nearly gone — only essential analysis calls are still being made.';
  else if (s.fraction >= QUOTA_CEILING.low) note = 'Over half the budget used — optional refreshes (price ticks, correlation) are paused.';
  else note = 'Within budget.';
  return { used: s.used, cap: s.cap, remaining: s.remaining, pct, note };
}
