...
import { isMarketOpen, marketClock, marketHoursBetween } from './auditor.js';
...
export function paperAccountSummary(positions, startingBalance, price) {
  ...
  // established.
  const graded = closed.filter(p => p.outcome === 'won' || p.outcome === 'lost'
    || (p.outcome == null && (p.pnl || 0) !== 0));   // records from before outcomes were tagged
