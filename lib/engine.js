...
import { isMarketOpen, marketClock, marketHoursBetween, nextMarketClose } from './auditor.js';
...
export function paperAccountSummary(positions, startingBalance, price, epoch) {
  const all = positions || [];
  const since = isFinite(epoch) && epoch > 0 ? epoch : null;
  const stamp = (p) => {
    const t = Date.parse(p.closedAt || p.openedAt || '');
    return isFinite(t) ? t : 0;
  };
  const inEra = (p) => since == null || stamp(p) >= since;
  const era = all.filter(inEra);
  const archived = all.filter(p => !inEra(p));
...
export function weekendFlattenState(now, cfg) {
  ...
  if (!cfg.enabled) return { flatten: false, closed: !isMarketOpen(t), minutesUntilClose: null, closesAt: null };
  ...
  if (!isMarketOpen(t)) {
    return { flatten: true, closed: true, minutesUntilClose: 0, closesAt: null };
  }
...
export function signalsToFlattenForWeekend(signals, state) {
  if (!state || !state.flatten) return [];
  const mins = Math.max(0, Math.round(state.minutesUntilClose || 0));
  const when = state.closed
  ...
  return (signals || [])
    ...
    .map(s => ({
      id: s.id,
      status: s.status,
      reason: (s.status === 'open' ? 'closed at market ' : 'order cancelled ') + when +
